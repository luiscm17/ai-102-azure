...
def main():
    ...
    try:
        ...
        image_file = os.path.join(current_dir, 'images/street.jpg')
        if len(sys.argv) > 1:
        ...
        if result.dense_captions is not None:
            ...
            for caption in result.dense_captions.list:
                print(" Caption: '{}' (confidense: {:2f}%)".format(caption.text, caption.confidence * 100))

        # Get image tags
        if result.tags is not None:
            print("\nTags:")
            for tag in result.tags.list:
                print(" Tag: '{}' (confidense: {:.2f}%)".format(tag.name, tag.confidence * 100))
...
def show_objects(image_filename, detected_objects):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    objectfile = os.path.join(current_dir, 'downloads/objects.jpg')
...
def show_people(image_filename, detected_people):
    ...
    plt.tight_layout(pad=0)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    peoplefile = os.path.join(current_dir, 'downloads/people.jpg')
