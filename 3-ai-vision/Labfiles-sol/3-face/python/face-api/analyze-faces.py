...
from azure.ai.vision.face.models import FaceDetectionModel, FaceAttributeTypeDetection01, FaceRecognitionModel
...
def main():
    ...
    try:
        ...
        image_file = os.path.join(current_dir, 'images', 'face1.jpg')
        if len(sys.argv) > 1:
...
def annotate_faces(image_file, detected_faces):
    ...
    plt.imshow(image)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    outputfile = os.path.join(current_dir, 'output', 'detected_faces1.jpg')
